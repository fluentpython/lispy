;;; $Id: chap5d.scm,v 4.0 1995/07/10 06:51:27 queinnec Exp $
...
(define (meaning-dotted-abstraction n* n e+)
  (let ((m1 (meaning-regular-variables n*))
        (m2 (meaning*-sequence e+))
        (m3 (meaning-dotted-variable n))
        (arity (length n*)) )
    (lambda (r k s)
      (k (inValue (lambda (v* k1 s1)
                    (if (>= (length v*) arity)
                        ((m1 (m3 (lambda (v* r k s) (m2 r k s))))
                         v* r k1 s1 )
                        (wrong "Incorrect arity") ) ))
         s ) ) ) )
